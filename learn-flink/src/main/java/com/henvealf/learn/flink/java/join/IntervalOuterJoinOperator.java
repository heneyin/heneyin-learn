 ...
 */
@Internal
public class IntervalOuterJoinOperator<K, T1, T2, OUT>
    ...
    private static final Logger logger = LoggerFactory.getLogger(IntervalOuterJoinOperator.class);
    ...
    private final IntervalOuterJoin.JoinType joinType;
     ...
     * @param joinType
     */
    public IntervalOuterJoinOperator(
            long lowerBound,
            long upperBound,
            ...
            IntervalOuterJoin.JoinType joinType) {
    ...
    private <THIS, OTHER> void processElement(
            ...
            final MapState<Long, List<IntervalOuterJoinOperator.BufferEntry<THIS>>> ourBuffer,
            final MapState<Long, List<IntervalOuterJoinOperator.BufferEntry<OTHER>>> otherBuffer,
        ...
        int ourIndex = addToBuffer(ourBuffer, ourValue, ourTimestamp);
        ...
        for (Map.Entry<Long, List<BufferEntry<OTHER>>> bucket : otherBuffer.entries()) {
            ...
            for (BufferEntry<OTHER> entry : bucket.getValue()) {
                ...
                setAsJoined(ourBuffer, ourTimestamp, ourIndex);
                entry.hasBeenJoined = true;
                joined = true;
                break;
//                hasJoined = true;
//                if (isLeft) {
//                    if (joinType == IntervalOuterJoin.JoinType.OUTER || joinType == IntervalOuterJoin.JoinType.LEFT) {
//                         collect((T1) ourValue, (T2) entry.element, ourTimestamp, timestamp);
//                    }
//                } else {
//                    if (joinType == IntervalOuterJoin.JoinType.OUTER || joinType == IntervalOuterJoin.JoinType.RIGHT) {
//                        collect((T1) entry.element, (T2) ourValue, timestamp, ourTimestamp);
//                    }
//                }
            }
            otherBuffer.put(timestamp, bucket.getValue());
            if (joined) {
                break;
            }
        }
...
//            if (isLeft) {
//                if (joinType == IntervalOuterJoin.JoinType.OUTER || joinType == IntervalOuterJoin.JoinType.LEFT) {
//                    collect((T1) ourValue, null, ourTimestamp, -1);
//                }
//            } else {
//                if (joinType == IntervalOuterJoin.JoinType.OUTER || joinType == IntervalOuterJoin.JoinType.RIGHT) {
//                    collect(null, (T2) ourValue, -1, ourTimestamp);
//                }
//            }
//        }
     ...
     * @return 在 list 中的下标
     */
    private static <T> int addToBuffer(
            final MapState<Long, List<IntervalOuterJoinOperator.BufferEntry<T>>> buffer,
            final T value,
        ...
        return elemsInBucket.size() - 1;
    }

    private static <T> void setAsJoined(final MapState<Long, List<IntervalOuterJoinOperator.BufferEntry<T>>> buffer,
                                       final long timestamp,
                                       final int index) throws Exception {
        List<BufferEntry<T>> elemsInBucket = buffer.get(timestamp);
        if (elemsInBucket == null || elemsInBucket.size() < index) {
            return;
        }
        BufferEntry<T> bufferEntry = elemsInBucket.get(index);
        elemsInBucket.set(index, new BufferEntry<>(bufferEntry.element, true));
        buffer.put(timestamp, elemsInBucket);
    }
            ...
            case CLEANUP_NAMESPACE_LEFT:
                {
                    ...
                    logger.info("移除前的 left buffer 数据：{}", leftBuffer.entries());
                    List<BufferEntry<T1>> bufferEntries = leftBuffer.get(timestamp);
                    logger.info("Removing from left buffer @ {}, values: {}", timestamp, bufferEntries);
                    for (BufferEntry<T1> bufferEntry : bufferEntries) {
                        if (!bufferEntry.hasBeenJoined) {
                            collect(bufferEntry.element, null, timestamp, -1);
                        }
                    }
            ...
            case CLEANUP_NAMESPACE_RIGHT:
                {
                    ...
                    logger.info("移除前的 right buffer 数据：{}", rightBuffer.entries());
                    List<BufferEntry<T2>> bufferEntries = rightBuffer.get(timestamp);
                    logger.info("Removing from right buffer @ {}, values: {}", timestamp, bufferEntries);
                    for (BufferEntry<T2> bufferEntry : bufferEntries) {
                        if (!bufferEntry.hasBeenJoined) {
                            collect(null, bufferEntry.element, -1, timestamp);
                        }
                    }
