...
public class MyJsonLayout extends LayoutBase<ILoggingEvent> {
    ...
    public String doLayout(ILoggingEvent iLoggingEvent) {
        ...
        jsonObject.put("logLevel", iLoggingEvent.getLevel().toString().toLowerCase());

        String message = iLoggingEvent.getMessage();
        String logDesc = iLoggingEvent.getMessage().substring(0, Math.min(message.length() - 1, 500));
        jsonObject.put("logDesc", logDesc);
